...
- [`art/starters/`](art/starters/) — Sproutling, Sparkpup, and Cottonwisp (`starters-v1.3-anemononima`, Anêmonônima Blob/Slime, CC0). Pick and title use the 140-class frames (canvas 220×200). The board uses the 64-class frames (canvas 96×80). Tight lists use the static 32. Idle is 8 frames at 5 FPS on one shared clock, blink on frame 07. Merge is a 6-frame one-shot at 9 FPS, then idle_00. Nearest filter, no scale tween on the frames. Every other shop species stays a capsule. `assets/drops/starters-v1.2-face/` and `assets/drops/starters-v1.1-subtle/` keep the previous sheets.

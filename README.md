...
- Title — `art/style-lock/title-menu-v1/`. The logo is `wordmark-hatchline-trio-512.png` (smaller trio above the word). Default background is `bg-title-texture-B-meadow-1280x800.png`; that path stays so a later sheet can replace it. Paper A is `TITLE_TEXTURE_CHOICE`. The muddy meadow is archived and is not shown. Play, Hatch-dex, and Options stay in the cream sky, above 42% of the screen.

...
- [`art/tokens/`](art/tokens/) — one capsule per family and tier (`leaf_t1.png` … `puff_t3.png`), cut from those sheets. `beast_t1.png` is the plain rose pill from the combat mock. Nest Warden, Bramble Bit, and Sprig Add still use their capsule crops. The old Meadow Mite and Tired Mite capsules are archived in `art/tokens/archive/`.
- [`art/starters/`](art/starters/) — Sproutling, Sparkpup, and Cottonwisp (Anêmonônima Blob/Slime, CC0). Pick and title use the 140-class frames (canvas 220×200). The board uses the 64-class frames (canvas 96×80). Tight lists use the static 32. Idle is `starters-v1.3.1-blink`: 8 frames at 5 FPS on one shared clock, half-lid on 05, closed hold on identical frames 06 and 07. Merge stays the v1.3 6-frame one-shot at 9 FPS, then idle_00. Nearest filter, no scale tween on the frames. Every other shop species stays a capsule. `assets/drops/starters-v1.3-anemononima/` keeps the previous idle. `assets/drops/starters-v1.2-face/` and `assets/drops/starters-v1.1-subtle/` keep the older sheets.
- [`art/wild/`](art/wild/) — Meadow Mite, Tired Mite, Barkling, Pollen Wisp, and Budmite (`wild-cast-v1-anemononima`, same Anêmonônima pack, CC0). Board, bench, combat, and dex use the 64-class frames (canvas 96×88). Budmite's unlock hero and shop portrait use the 140-class frames (canvas 220×220). Idle is the same 8-frame shared clock at 5 FPS, nearest, no scale tween. No merge sheet and no telegraph. The drop archive is `assets/drops/wild-cast-v1-anemononima/`.
...
- Title — `art/style-lock/title-menu-v1/composite/`. The default is the pack stack (sky, drifting clouds, far, mid, near, a light paper veil). The logo is the live starter trio above `wordmark-hatchline-only-512.png`, high in the sky. The type does not bob. Play, Hatch-dex, and Options are a vertical stack of cream pills. The first pill is about 58% from the top. They grow to 1.03 on hover. Meadow texture B is the Art-meadow fallback. `title-composite-painted-alt.png` is the painted flip (`TITLE_BG_CHOICE := "painted"`).
- Starter select — `art/style-lock/starter-select-v1/`. Play opens three cream cards on the same pack stack. Sparkpup starts selected. Confirm calls `choose_starter`; Back returns to the title. Portraits idle from the 8-frame Anêmonônima sheets.

Sky art is edermunizz, [Simple Pastel Backgrounds](https://edermunizz.itch.io/free-simple-pastel-backgrounds) (attribution required). Hill layers are najjar320 VISTA (CC0). Clouds are GarzettDev. The paper veil is CC0. The painted alternate, if used, is heartpunch (CC0). Starter bodies, meadow enemies, and Budmite are Anêmonônima, [Blob/Slime collection](https://opengameart.org/content/blobslime-collection) (CC0). Family marks and the wild-cast identity marks are Hatchline Art.
...
2. **Sparring** is the teaching fight. Press Fight and watch the two boards. A melee critter lunges. A ranged critter spits a seed. The target gives a small pop, and the number floats on the cell. A kill still pops. Speed toggles between ×1 and ×2. During the fight the combat log keeps the latest hits on screen, including at ×2. View full log opens a longer tail. After the fight the full log stays on screen in a scroller. A starter in the right column wins. A ranged critter standing in Front, or a melee critter stuck in Back, can lose.
3. The **Meadow Stall** opens next. Buy, Freeze, Reroll, and sell. Two copies of your starter fill the first slots. Buying both triples it. Onward finishes that triple if it is still short, so a clean starter reaches Wild Grass at 3/3. Buy and Reroll show the coin cost after a middot (`Buy · 3`, `Reroll · 1`). Budmite still joins after any finished run, including a loss.
